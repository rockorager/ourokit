...
//! to texel centers; edges clamp. Filtering stays in premultiplied encoded sRGB.
const std = @import("std");
const scene = @import("../scene/root.zig");
const Bitmap = @import("../image/pixels.zig").Bitmap;
const Pixel = @import("../core/color.zig").PremultipliedSrgba8;
...
pub const Placement = struct {
    ...
    pub fn sample(self: Placement, bitmap: *const Bitmap, x: usize, y: usize) ?Pixel {
        ...
        for (&channels, 0..) |*value, channel| {
            const a: u32 = bitmap.pixels[(y0 * bitmap.width + x0) * 4 + channel];
            const b: u32 = bitmap.pixels[(y0 * bitmap.width + x1) * 4 + channel];
            const c: u32 = bitmap.pixels[(y1 * bitmap.width + x0) * 4 + channel];
            const d: u32 = bitmap.pixels[(y1 * bitmap.width + x1) * 4 + channel];
