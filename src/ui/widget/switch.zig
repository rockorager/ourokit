...
pub const Switch = struct {
    ...
    pub fn init(theme: tokens.Theme, checked: bool, enabled: bool, radius: ?f32) Switch {
        ...
        const border = if (enabled and checked) theme.primary else theme.switch_border;
...
test "Switch recipe uses semantic colors and asymmetric checked positions" {
    ...
    for ([_]tokens.Theme{ tokens.light, tokens.dark }) |theme| {
        for ([_]bool{ false, true }) |checked| {
            const inactive = Switch.init(theme, checked, false, null);
            try std.testing.expectEqual(theme.switch_border, inactive.track.border_color.?);
            try std.testing.expectEqual(theme.switch_border, inactive.thumb.border_color.?);
            try std.testing.expectEqual(theme.disabled, inactive.track.background.?);
        }
    }
}
